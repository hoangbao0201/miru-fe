...
interface FeatureTeamplateProps {
    title: string;
...
export default function FeatureTeamplate({
    title,
    content,
    options,
}: FeatureTeamplateProps) {
    return (
        <>
...
async function BooksView({
    ...
    content: ContentPageEnum;
    options: FeatureTeamplateProps["options"];
}) {
    const currentDate = new Date();
    const querys = new URLSearchParams({
        take: "24",
        page: String(options.page),
        isFeatured: String(options.isFeatured),
        ...(content && {
            category: content,
        }),
    });
